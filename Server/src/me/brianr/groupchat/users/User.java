...
import me.brianr.groupchat.server.filemanager.MySQLManager;
...
//	protected boolean isLoggedIn;
	...
	public static User getUserFromDatabase(long id) throws SQLException {
		...
		while (results.next()) {
			if (results.getInt("id") == id) {
				...
				long uid = results.getLong("id");
				Role role = Role.valueOf(results.getString("role"));
				
				User thatUser = new User(name, displayName, uid, role);
				return thatUser;
			}
		}
